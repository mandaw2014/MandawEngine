...
class Player(Sprite):
    ...
    def movement(self):
        # Player movement
        if self.window.controls.is_key_pressed(self.window.keys["A"]):
            if self.enabled == True:
                self.pos_x -= self.speed * self.window.dt
                self.direction = 0
                self.play_animation("attack")

        if self.window.controls.is_key_pressed(self.window.keys["D"]):
            if self.enabled == True:
                self.pos_x += self.speed * self.window.dt
                self.direction = 1
        ...
        if self.collide(self.objects):
            ...
            if self.direction == 0 and not self.window.controls.is_key_pressed(self.window.keys["A"]):
                self.pos_x += 10 * self.window.dt

                if self.pos_x >= 0:
                    self.pos_x = 0
            
            if self.direction == 1 and not self.window.controls.is_key_pressed(self.window.keys["D"]):
                self.pos_x -= 10 * self.window.dt
    ...
    def jump(self):
        ...
        if self.is_jumping == False and self.window.controls.is_key_pressed(self.window.keys["SPACE"]):
