'''
Original code:
https://github.com/idgmatrix/pygame-physics/blob/master/verlet_cloth_system_mouse2.py
@author: kaswan
...
class Particle:
    ...
    def update(self, delta_t):
        ...
        if self.selected == True:
            self.x, self.y = mandaw.input.get_mouse_pos()
...
class Constraint:
    ...
    def draw(self, surf, size):
        ...
        Line(mandaw, WHITE, (int(x0), int(y0)), (int(x1), int(y1)))
...
while True:
    ...
    # particles draw
    for i in range(len(particles)):
        particles[i].draw(mandaw.window, 3)

    # constraints draw
    for i in range(len(constraints)):
        constraints[i].draw(mandaw.window, 1)
